...
def update(self):
    ...
    action_batch = torch.LongTensor(action_batch).reshape(self.batch_size, self.num_agent).to(self.device)
    ...
    q_vals = self.critic(state_batch, mac_out.view(self.batch_size, self.num_agent, -1))

    # Compute the counterfactual baseline
    pi = mac_out.view(self.batch_size, self.num_agent, self.act_dim)
    baseline = (pi * q_vals).sum(-1).detach()

    # Compute the COMA loss
    q_taken = torch.gather(q_vals, dim=2, index=action_batch.unsqueeze(-1)).squeeze(-1)
    pi_taken = torch.gather(pi, dim=2, index=action_batch.unsqueeze(-1)).squeeze(-1)
