...
class COMA:
    ...
    def update(self):
        ...
        state_batch, action_batch, reward_batch, next_state_batch, done_batch, avail_action_batch = self.replay_buffer.get_batches()
        ...
        avail_action_batch = torch.Tensor(avail_action_batch).reshape(self.batch_size, self.num_agent, self.act_dim).to(self.device)  # ADDED
        ...
        mac_out[avail_action_batch == 0] = 0
        mac_out = mac_out / mac_out.sum(dim=-1, keepdim=True)
        mac_out[avail_action_batch == 0] = 0
        pi = mac_out.view(-1, self.act_dim)
        baseline = (pi * main_q.view(-1, self.act_dim)).sum(-1).detach()
